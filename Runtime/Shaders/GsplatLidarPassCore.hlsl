...
#define GSPLAT_LIDAR_PASS_CORE_INCLUDED
            ...
            StructuredBuffer<uint> _LidarExternalRangeSqBits;
            StructuredBuffer<float4> _LidarExternalBaseColor;
            ...
            v2f vert(appdata v)
            {
                ...
                uint splatRangeSqBits = _LidarMinRangeSqBits[cellId];
                uint externalRangeSqBits = _LidarExternalRangeSqBits[cellId];
                bool hasSplatHit = splatRangeSqBits < kLidarInfBits;
                bool hasExternalHit = externalRangeSqBits < kLidarInfBits;
                if (!hasSplatHit && !hasExternalHit)
                {
                    o.vertex = float4(0.0, 0.0, 2.0, 1.0);
                    return o;
                }

                bool useExternalHit = hasExternalHit && (!hasSplatHit || externalRangeSqBits < splatRangeSqBits);
                ...
                if (colorBlend > 1.0e-4)
                {
                    if (useExternalHit)
                    {
                        hitRgb = _LidarExternalBaseColor[cellId].rgb;
                    }
                    else
                    {
                        ...
                        if (splatId != kLidarInvalidId)
                        {
                            ...
                            hitRgb = sh0.rgb * SH_C0 + 0.5;
                        }
                    }
                }
                float3 baseRgb = lerp(depthRgb, hitRgb, colorBlend);
                ...
                // 扫描前沿+余辉(按真实方位角年龄):
                // - frustum 模式下 active bins 更少,但 RotationHz 的“全局旋转频率”语义不能偷偷变快.
                // - 因此这里不直接用 active bins 去算 headBin,而是:
...
#endif // GSPLAT_LIDAR_PASS_CORE_INCLUDED
