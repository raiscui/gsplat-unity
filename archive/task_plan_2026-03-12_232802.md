...
## 2026-03-12 15:30:00 +0800 新任务: continuous-learning 沉淀本轮单帧 PLY / importer 经验

### 目标

- [ ] 列出并分组回读默认六文件与支线六文件
- [ ] 产出六文件摘要,判断知识沉淀载体
- [ ] 同步更新 README / AGENTS / 相关文档或 skill
- [ ] 回写 WORKLOG / notes / task_plan,完成 continuous-learning 闭环

### 当前状态

**目前在阶段3(知识落盘)**
- 已完成默认组与 4 个支线组的最新段落回读.
- 已确认一个必须同步的过期点: `README.md` 仍把 `.ply` 导入结果写成 `Gsplat Asset`,与当前 prefab main object 行为冲突.
- 正在同步 README 与 `AGENTS.md`,随后再决定是否新增 self-learning skill.

## 2026-03-12 15:45:00 +0800 追加问题: 判断 `ckpt_29999_v2_sh3_seg50_k512_f32_colmap_latest.splat4d` 是否存在 `f_rest_*` 读取顺序问题

### 目标

- [ ] 检索现有 notes / ERRORFIX / WORKLOG 中是否已有该资产的直接证据
- [ ] 如无直接证据,补做最小离线验证,区分 `channel-major` 与 `interleaved`
- [ ] 给出分级结论: 现象 / 假设 / 验证证据 / 结论

### 当前状态

**目前在证据检索阶段**
- 先查仓库内是否已经记录过这份 `ckpt_29999_v2_sh3_seg50_k512_f32_colmap_latest.splat4d` 的 SH 排列结论,避免重复做实验.

## 2026-03-12 15:52:00 +0800 进展: `ckpt_29999...` 的 `f_rest_*` 排列问题完成边界判断

- [x] 检索现有 notes / ERRORFIX / WORKLOG 中是否已有该资产的直接证据
- [x] 在无直接对比证据的情况下,先给出基于生成链路的边界判断
- [ ] 如用户需要,再补做 `ckpt_29999...` 的离线 SH 反解对比实验

### 当前状态

**目前已得到第一层结论**
- `ckpt_29999...` 是多帧 `.splat4d v2` 资产,不是当前单帧 v2 exporter 直接产物.
- 当前没有证据能证明它“没有”历史 SH 排列问题.
- 但也不能把 `s1` 已确认的单帧 exporter bug 直接外推到它身上.

## 2026-03-12 16:02:00 +0800 追加问题: 检查 `FreeTimeGsVanilla` 多帧 exporter 是否也存在 `f_rest_*` 读取顺序错误

### 目标

- [ ] 检索 `/Users/cuiluming/local_doc/l_dev/my/python/FreeTimeGsVanilla` 中所有 `.splat4d` / SH 导出代码路径
- [ ] 核对 `f_rest_*` / SH rest 的读取与重排逻辑,判断是否按 `RRR... GGG... BBB...` 还是 `RGBRGB...`
- [ ] 给出证据化结论,区分“现象 / 假设 / 代码证据 / 结论”

### 当前状态

**目前在代码路径排查阶段**
- 先从 `FreeTimeGsVanilla` 里定位多帧 `.splat4d v2` 的实际导出脚本,再判断它是否复用了同样的 SH 排列错误.

## [2026-03-12 16:10:00 +0800] 进展: 开始核对 FreeTimeGsVanilla 多帧 `.splat4d` 导出链路

### 目标

- [ ] 读取 FreeTimeGsVanilla 导出器相关代码与映射文档
- [ ] 核对它读取的源数据是否为 PLY `f_rest_*`
- [ ] 核对导出布局与 Unity 导入布局是否一致
- [ ] 给出基于证据的结论, 明确边界

### 当前状态

**目前在代码与映射核对阶段**
- 已定位 `tools/exportor/export_splat4d.py` 是多帧 `.splat4d` 导出脚本.
- 下一步读取 `shN` 布局推断、per-band flatten 和 `SHCT` 写出路径, 判断它是否复用了单帧 PLY exporter 的 `f_rest_*` 读取错误.

## [2026-03-12 16:15:00 +0800] 进展: 已完成 FreeTimeGsVanilla 多帧 exporter 的同类 bug 静态核对

### 结果

- [x] 读取 FreeTimeGsVanilla 导出器相关代码与映射文档
- [x] 核对它读取的源数据是否为 PLY `f_rest_*`
- [x] 核对导出布局与 Unity 导入布局是否一致
- [x] 给出基于证据的第一层结论,并保留动态数据验证入口

### 当前状态

**目前已完成静态证据链闭环**
- `FreeTimeGsVanilla/tools/exportor/export_splat4d.py` 的 SH 来源是 checkpoint `splats["shN"]`,不是 PLY `f_rest_*`.
- 静态与动态 `shN` flatten 都保持 `[coeff][rgb]` 邻接布局.
- Unity importer 读取 `SHCT` 时也按每个 coeff 对应一个 `Vector3(r,g,b)` 解释.
- 因此当前没有证据表明它存在“与单帧 PLY exporter 相同的读取顺序错误”.
- 如果后续还要继续深挖,下一步应做 `ckpt/asset` 的离线数据对照,而不是继续围绕 PLY `f_rest_*` 假设打转.

## [2026-03-12 16:22:00 +0800] 追问: 核对 SHCT 写入 layout 与 Unity 读取 layout 是否逐元素一致

### 目标

- [ ] 读取 `FreeTimeGsVanilla` 中 SHCT codebook 的内存布局定义
- [ ] 对照 Unity importer 的 `SHCT` 解码索引公式
- [ ] 给出 layout 是否一致的结论, 并说明边界

### 当前状态

**目前在 layout 逐元素核对阶段**
- 这一步不再只回答“有没有同类 bug”,而是直接核对 `SHCT` payload 的字节布局与 Unity 读法是否一一对应.

## [2026-03-12 16:24:00 +0800] 进展: SHCT 写入 / 读取 layout 对齐核对完成

### 结果

- [x] 读取 `FreeTimeGsVanilla` 中 SHCT codebook 的内存布局定义
- [x] 对照 Unity importer 的 `SHCT` 解码索引公式
- [x] 得到静态结论: 写端与读端 layout 一致

### 当前状态

**目前已完成 layout 对齐结论**
- exporter 把每个 band 的 codebook 以 `[codebookEntry, coeffRGBFlat]` 的 C-order 连续字节写入 `SHCT`.
- 对单个 codebook entry 而言, 内层顺序是 `coeff0.rgb`, `coeff1.rgb`, `coeff2.rgb` ...
- Unity importer 也按 `(k * coeffCount + c) * 3 + rgb` 取值,逐个重建 `Vector3(r,g,b)`.
- 因此当前 SHCT 写 / 读契约本身是对齐的.

## [2026-03-12 16:30:00 +0800] 继续执行: 做 `ckpt_29999...` 的动态数据对照验证

### 目标

- [ ] 在 `FreeTimeGsVanilla` 工程中定位与 `ckpt_29999_v2_sh3_seg50_k512_f32_colmap_latest.splat4d` 对应的原始 checkpoint
- [ ] 读取 `.splat4d` 的 `SHCT/SHLB` sections,反解出若干 splat 的 SH rest
- [ ] 读取 ckpt 的 `splats["shN"]`,按 band/coeff 对齐后做逐项比对
- [ ] 给出基于动态证据的结论,确认是否存在 layout 或顺序错误

### 当前状态

**目前在动态验证准备阶段**
- 静态代码层已经确认 SHCT 写/读 layout 一致.
- 下一步直接找 `ckpt_29999...` 的源 ckpt 和导出链路产物,避免继续只靠静态阅读判断.

## [2026-03-12 16:37:00 +0800] 进展: FreeTimeGsVanilla 当前工作树未保留真实大 ckpt 产物路径

### 现象

- 历史 `notes/WORKLOG` 记录的 `results/bar_release_full/out_0_61/...` 在当前磁盘上不存在.

### 调整后的下一步

- [ ] 在更大范围内定位是否还有 `ckpt_29999.pt` 或同源导出目录残留
- [ ] 若找到 ckpt,继续做真实 `ckpt -> asset` 动态对照
- [ ] 若找不到 ckpt,转为“资产自证”验证: 解码真实 `.splat4d` 的 SHCT/SHLB/SHDL,确认 section 自洽且与 Unity importer 的索引契约一致

### 当前状态

**目前在证据路径补救阶段**
- 这不是根因结论,只是说明原始证据文件不在当前 FreeTimeGsVanilla 工作树里.

## [2026-03-12 16:43:00 +0800] 用户补充: 没有原始 ckpt,切换为真实 `.splat4d` 资产自证验证

### 目标

- [ ] 直接读取真实 `.splat4d` 资产 header + section table
- [ ] 用两套独立方式解码同一份 `SHCT` 字节:
  - reshape 视角: `[entry, coeff, rgb]`
  - Unity importer 视角: `((entry * coeffCount) + coeff) * 3 + rgb`
- [ ] 比较两者结果是否逐元素完全一致
- [ ] 结合 `SHLB/SHDL` 给出当前资产层面的结论

### 当前状态

**目前在资产自证阶段**
- 用户已确认没有原始 ckpt.
- 当前最强证据路径是直接对 `Assets/Gsplat/splat/v2/ckpt_29999_v2_sh3_seg50_k512_f32_colmap_latest.splat4d` 做离线反解与双重索引对照.
