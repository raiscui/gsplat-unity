...
## 2026-03-03 01:05:00 +0800 - RadarScan 下 Show/Hide 动画语义补齐

### 现象
- 进入 `RadarScan` 后,`Show/Hide` 只影响 splat 后端.
- LiDAR 点云只受 `_LidarVisibility`(雷达开关淡入淡出)影响,不会走 show/hide 的中心扩散语义.

### 根因
- `GsplatLidar.shader` 缺少 show/hide overlay uniforms 与掩码计算.
- `GsplatRenderer/GsplatSequenceRenderer` 在调用 `RenderPointCloud(...)` 时,没有把 `m_visibilityState/progress/sourceMask` 传入 LiDAR 链路.

### 修复策略
1. 在 `GsplatLidarScan.RenderPointCloud(...)` 扩展参数与 MPB:
   - `showHideGate/mode/progress/sourceMaskMode/sourceMaskProgress`
   - `showHideCenterModel/maxRadius/ringWidth/trailWidth`
   - `worldToModel` 矩阵(让 LiDAR 在 model 空间复用 show/hide 语义)
2. 在 `GsplatRenderer` 和 `GsplatSequenceRenderer` 新增 `BuildLidarShowHideOverlayForThisFrame(...)`,从当前 visibility runtime 状态生成 LiDAR overlay 参数.
3. 在 `GsplatLidar.shader` 新增 show/hide 掩码评估:
   - `mode=show`: `max(primary, source)`
   - `mode=hide`: `primary * source`
   - 支持 hide 起始小宽度 ramp,与 splat hide 起始观感一致.
4. Hidden 终态(`gate=0 && mode=0`)直接跳过 LiDAR draw,避免隐藏态仍提交无效点云.
...
## 2026-03-03 01:36:00 +0800 - RadarScan show/hide 缺少 noise 颗粒感

### 现象
- `RadarScan` 下 show/hide 已有中心扩散动画,但边界太干净.
- 与 `ParticleDots` 相比,缺少 noise 抖动和烧尽颗粒感.

### 根因
- C# 参数链路已存在(`VisibilityNoiseMode/NoiseStrength/NoiseScale/NoiseSpeed`).
- 但 `Runtime/Shaders/GsplatLidar.shader` 的 show/hide 计算仍在走旧调用:
  - `EvalLidarShowHideVisibleMask(...)` 调用未传 noise 参数.
  - source-mask 分支同样未传 noise 参数.
  - ring glow 边界也未使用 noise 版本函数.

### 修复要点
1. `EvalLidarShowHideVisibleMask(...)` 语义对齐 splat 侧:
- show/hide 都应用 `ashMul`.
- hide 仍保留 `passed^2` 的烧尽节奏.
2. `EvalLidarShowHideRingMask(...)` 增加 `mode` 参数:
- 使用 noiseWeight 做边界 jitter.
- ring 只保留外侧前沿(避免内外双边同时发光).
3. vert overlay 统一接线:
- 基于 `modelPos + _LidarTime * _LidarShowHideNoiseSpeed` 计算 `noise01/noiseSigned`.
- primary mask / source mask / ring glow 全部复用同一套 noise 结果.
...
- 新增测试:
  - `LidarRenderPointCloud_Signature_ContainsShowHideNoiseParams`
  - 目的: 锁定 `GsplatLidarScan.RenderPointCloud` 的 noise 参数契约,防止后续重构丢参.

## 2026-03-03 02:06:00 +0800 - RadarScan noise 二次增强(观感可见性)

### 二次诊断
- 参数链路无断点,问题不在“没传到 shader”.
- 根因在 LiDAR noise 公式保守:
  - 频率偏低,在常见模型尺度下变化太慢.
  - jitter 完全绑定 trailWidth,当 trailWidth 较小时噪声几乎被吃掉.

### 本次增强
- `EvalLidarShowHideNoise01`:
  - 提高基础采样频率.
  - `CurlSmoke` 模式增加轻量 domain warp + 双层 noise 混合.
- `EvalLidarShowHideVisibleMask` / `EvalLidarShowHideRingMask`:
  - jitter 改为 `max(trailWidth*0.75, maxRadius*0.015)` 下限策略.
  - 增加 `noiseWeight` 下限(`lerp(0.35,1.0,noiseWeight0)`),避免边界扰动被过度衰减.
- ring glow:
  - 增加 `noise01` 调制,让前沿明暗有粒子化起伏.
...
## 2026-03-03 02:14:00 +0800 - RadarScan noise 三次增强(默认参数可见)

- 进一步把 LiDAR noise 强度映射从 `x1.0` 提升到 `x1.6`(shader 内部),
  目标是默认 `NoiseStrength=0.6` 时也有明确可见的颗粒变化.
- 保持用户侧参数不变,仅调整 LiDAR 路径内部权重,不影响 splat 路径.
- 回归通过: `Gsplat.Tests` total=43, passed=41, failed=0, skipped=2.
...
## 2026-03-03 02:22:00 +0800 - 参考 ParticleDots 的噪声位移补齐

- 用户反馈"还是没有 noise"后,确认差异点不是参数传递,而是 LiDAR 缺少 ParticleDots 那种位置扰动质感.
- 本次改动:
  - 在 LiDAR show/hide 中新增屏幕空间粒子位移抖动(按 edge/ring/progress 加权),让边界有明显"噪声扰动".
  - 继续保留 primary/source/ring 的噪声遮罩语义.
- 结果: RadarScan 在 show/hide 过渡阶段不再只有明暗变化,而是有可见的颗粒位移感.
- 回归: total=43, passed=41, failed=0, skipped=2.
