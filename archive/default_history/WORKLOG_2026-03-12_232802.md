...
## [2026-03-12 16:15:00 +0800] 任务名称: 排查 FreeTimeGsVanilla 多帧 `.splat4d` 是否复用单帧 PLY 的 `f_rest_*` 读取错误

### 任务内容
- 核对 `FreeTimeGsVanilla` 的多帧 `.splat4d` 导出链路.
- 判断它是否也经历了“PLY `f_rest_*` -> `.splat4d`”这条已知有坑的路径.
- 对照 Unity importer 的 `SHCT` 读取布局,确认导出/导入契约是否一致.

### 完成过程
- 读取了 `tools/exportor/export_splat4d.py` 的 `shN` 来源、布局推断、静态 per-band flatten、动态按帧采样与 labels 分配路径.
- 对照 `Editor/GsplatSplat4DImporter.cs` 的 `SHCT` 解码逻辑,确认 importer 以 `(label * coeffCount + coeff) * 3` 读成 `Vector3(r,g,b)`.
- 额外检索了 `FreeTimeGsVanilla/tools/exportor` 与相关 spec,没有看到 PLY / `f_rest_*` 的读取路径命中.

### 总结感悟
- 这条多帧导出链路的 SH 风险点不再是“PLY 属性顺序读反了”,而是“checkpoint `shN` 的语义是否和 exporter/importer 契约一致”.
- 下一次如果要继续验证,最值钱的动作不是再读代码,而是拿原始 ckpt 的 `shN` 和导出的 `SHCT/SHLB` 做离线逐样本对照.

## [2026-03-12 16:47:00 +0800] 任务名称: 在无原始 ckpt 的前提下对真实 `.splat4d` 做 SHCT 资产自证验证

### 任务内容
- 在没有原始 `ckpt_29999.pt` 的情况下,直接验证真实 `.splat4d` 资产的 `SHCT` 写读契约是否一致.
- 用两套独立索引方式解码同一份 `SHCT` 字节,避免只依赖单边源码解释.

### 完成过程
- 读取了真实资产 `Assets/Gsplat/splat/v2/ckpt_29999_v2_sh3_seg50_k512_f32_colmap_latest.splat4d` 的 header 与 section table.
- 解析得到:
  - `splat_count = 1335131`
  - `sh_bands = 3`
  - `frame_count = 61`
  - SH 分段为 `(0,50)` 与 `(50,11)`
- 对 band1/band2/band3 的 `SHCT` 分别执行:
  - reshape 解码 `[entry, coeff, rgb]`
  - Unity importer 解码 `((entry * coeffCount) + coeff) * 3 + rgb`
- 三个 band 的最大绝对差值都为 `0.0`,说明真实资产层面的 `SHCT` layout 契约完全对齐.

### 总结感悟
- 当上游原始 ckpt 已经丢失时,最稳妥的办法不是停止分析,而是把“格式层自证”做到足够强.
- 这次最有价值的证据不是“代码看起来一样”,而是“同一份真实字节被两套独立索引解码后逐元素完全相等”.
