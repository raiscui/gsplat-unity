...
## 2026-02-23: ActiveCameraOnly(单相机)排序/渲染的性能优化思路

### 现象(性能瓶颈的“本质”)

- 当前 SRP 路径使用 `RenderPipelineManager.beginCameraRendering` 作为“按相机触发 sort”的统一入口.
- 这能保证 SceneView 正确,但也意味着:
  - 场景里每多一个相机(反射/探针/多视口/辅助 camera),就可能在同一帧多触发一次 sort.
  - 在 >1M 且 <10M 的规模下,sort 的 GPU 成本足以让帧率线性恶化.

### 关键洞察

- 透明正确性要求“排序与渲染必须基于同一个相机”.
- 但大多数项目在同一时刻,其实只需要“一个窗口/一个视角”正确:
  - EditMode 你在看 SceneView,就只要 SceneView 正确.
  - PlayMode 你在看 GameView,就只要 GameView(主相机)正确.
- 因此最佳杠杆不是换语言(Rust),而是把“需要排序+渲染的相机数量”压到 1.

### 方案草案(将固化到 OpenSpec)

1. 引入 settings: `CameraMode = ActiveCameraOnly | AllCameras`.
2. 增加 ActiveCamera 解析规则:
   - Play 模式(含 Player build): 总是选 Game/VR 相机.
     - 多相机时优先 `Camera.main`,否则选 depth 更大的那个.
   - Editor 非 Play:
     - SceneView 聚焦 -> 选 `SceneView.lastActiveSceneView.camera`.
     - GameView 聚焦 -> 选 `Camera.main`(或自动选出的 Game 相机).
3. 在 sorter 侧门禁:
   - 非 ActiveCamera 的相机,`GatherGsplatsForCamera` 直接返回 false,从源头阻止 sort.
4. 在 render 侧门禁:
   - `Graphics.RenderMeshPrimitives` 只对 ActiveCamera 提交,其它相机不渲染 Gsplat,从而避免“看到了但没 sort”这种错误组合.

### Editor 焦点判定(可行但要小心)

- SceneView: `SceneView.lastActiveSceneView != null && lastActiveSceneView.hasFocus`.
- GameView: `EditorWindow.focusedWindow?.GetType().Name == \"GameView\"`(GameView 类型是 internal,只能用 Name 判断).
- 风险: 多 SceneView 窗口时,`lastActiveSceneView` 未必等于当前渲染的那个.
  - 可选增强: 遍历 `SceneView.sceneViews`,找 `hasFocus==true` 的那个,再匹配 camera.

### 本次落盘(证据)

- 已创建并完成 OpenSpec change:
...
## 2026-02-23 18:53:24 +0800: ActiveCameraOnly 落地后的“踩坑点”与稳态策略

### 1) `-batchmode -nographics` 下的相机枚举不稳定

- 现象:
  - 在 Unity `-batchmode -nographics` 的 EditMode tests 里,`TryGetActiveCamera` 可能返回 false.
  - 这会导致 ActiveCameraOnly 下直接不渲染,同时也会击穿回归测试.
- 根因:
  - 在少数环境下,`Camera.allCamerasCount` 可能返回 0,即使场景里确实存在 Camera.
- 稳态修复:
  - 当 `Camera.allCamerasCount==0` 时,用 FindObjects 系列做一次兜底枚举.
  - 正常情况下仍走 `Camera.GetAllCameras` 的无 GC 快路径.

### 2) EditMode tests 不能假设“场景里只有我创建的相机”

- 现象:
  - 测试项目可能默认打开一个包含 `Main Camera` 的场景.
  - 用例里的“单相机”假设会被污染,导致断言失败.
- 稳态修复:
  - 在 `[SetUp]` 里创建 `EmptyScene`,让每个用例的相机集合可控且可复现.

### 3) 设计复盘: 性能优化的关键不是 Rust,是缩小工作量

- 对 >1M 且 <10M splats:
  - GPU sort 的成本远高于 C# 侧枚举相机的开销.
  - 真正的性能杠杆是把“每帧需要 sort+render 的相机数量”压到 1.

---

## 2026-02-23 19:34:58 +0800: Editor UI 交互导致 SceneView 闪烁的根因与修复

### 现象
- Editor 非 Play 模式下.
- 当你在 Inspector/Hierarchy 等 UI 中交互时,SceneView 中的 Gsplat 会“显示/不显示”来回闪烁.

### 根因
- `CameraMode=ActiveCameraOnly` 后,渲染与排序都被门禁到 ActiveCamera.
- 旧 EditMode 规则过于“严格依赖焦点”:
  - SceneView 必须 `hasFocus==true` 才会被选为 ActiveCamera.
  - UI 交互时 SceneView 往往失焦,ActiveCamera 会被解析为 Game/VR 相机或上一帧缓存.
- 结果是:
  - SceneView 在 repaint 时并不一定是 ActiveCamera.
  - 由于 Render 也只提交给 ActiveCamera,SceneView 就会出现“有时画,有时不画”的整体闪烁.

### 修复策略(更符合编辑器直觉)
- EditMode 下:
  - 只有当 GameView 聚焦时,才切到 Game/VR 相机.
  - 否则只要 SceneView 存在,就优先使用 SceneView 相机(不要求 hasFocus).
- 这会让你在 Inspector 里拖参数时,SceneView 始终是主视角,因此不再闪烁.

---

## 2026-02-23 21:17:00 +0800: focusedWindow 信号不可靠,需要 viewport hint 缓存

### 新发现
- 用户反馈: 在 `ActiveCameraOnly` 下,即便我们“EditMode 默认优先 SceneView”,仍然会在与 UI 交互时闪烁.
- 说明 `EditorWindow.focusedWindow` 这类信号在真实交互链路里可能抖动,或者停留在 GameView/SceneView,
  不能作为“你正在看哪个视口”的唯一依据.

### 修复要点
- 用 `EditorWindow.mouseOverWindow` 作为主信号:
  - 鼠标在哪个视口上,就认为你正在看哪个视口.
- 当鼠标在 Inspector/Hierarchy 等非视口窗口上交互时:
  - 沿用上一帧的 hint,不要切换 ActiveCamera,避免闪烁.
- 同步修复测试:
  - EditMode 下存在 SceneView 时,`TryGetActiveCamera` 可能返回 SceneView 相机,因此“Game/VR 自动选择规则”的测试应改为直接验证内部选择算法(反射调用),避免依赖 Editor UI 状态.

---

## 2026-02-24 00:13:39 +0800: 闪烁仍存在时的进一步稳态修复(补强 SceneView 锁定)

### 新现象(用户反馈)
- 转动/移动 SceneView 视角时,鼠标在 UI 上划过,SceneView 内仍会出现“显示/不显示”闪烁.

### 推断根因(工程直觉)
- `ActiveCameraOnly` 的闪烁本质是“ActiveCamera 在短时间内被解析到非 SceneView 相机”,导致 SceneView 那一帧没有提交 draw.
- 仅靠 `EditorWindow.focusedWindow/mouseOverWindow` + 缓存,依然会遇到两类不可靠场景:
  - 鼠标锁定/拖拽中导致 `mouseOverWindow==null`,此时用 focusedWindow 更新 hint 会把状态抖动到 GameView.
  - SceneView 相机 transform 的更新时间点不一定与脚本 Update 同步,导致“用 transform 变化检测交互”存在漏判.

### 本次补强策略
- 增加更可靠的 SceneView 交互信号:
  - 监听 `SceneView.duringSceneGui`.
  - 当出现 MouseDrag/ScrollWheel/KeyDown 等交互事件时,记录时间戳并强制 hint=SceneView.
- 把 `mouseOverWindow==null` 视为歧义状态:
  - 若已有缓存 hint,直接沿用,不再根据 focusedWindow 更新.
  - 目的: 避免 UI/鼠标锁定导致 hint 抖动,从根源消灭“显示/不显示”闪烁.

### 证据型验证
- Unity MCP: `Gsplat.Tests.Editor` total=22, passed=22, failed=0, skipped=0.

---

## 2026-02-24 06:18:43 +0800: Metal 缓冲区未绑定 warning 与命令行测试跑法踩坑

### Metal warning 线索
- Console warning:
  - `Metal: Vertex or Fragment Shader "Gsplat/Standard" requires a ComputeBuffer at index ... to be bound, but none provided. Skipping draw calls...`
- 推断:
  - 这是“shader 需要的 StructuredBuffer 没绑定”的硬错误.
  - Unity 为避免崩溃会直接跳过 draw call,从而表现为 GameView/SceneView 中的 Gsplat 偶发整体消失.

### 稳态修复要点
- 渲染侧(`Runtime/GsplatRendererImpl.cs`):
  - 每次 `Render()` 提交 draw call 前,重新 `SetBuffer` 绑定所有 StructuredBuffers,避免 Metal 下出现“绑定丢失 -> 跳绘制”.
  - 把 4D buffers(Velocity/Time/Duration)视为“始终需要绑定”的资源:
    - `Has4D=false` 时也创建 dummy buffer 并绑定,避免 shader 声明了 buffer 但运行时没绑.

### Unity 命令行跑 package tests 的经验
- 一个最小可复现项目(只装本包+test framework)默认不会编译包内 tests.
  - 需要在 test project 的 `Packages/manifest.json` 增加:
    - `"testables": ["wu.yize.gsplat"]`
- 命令行跑 `-runTests` 时不要加 `-quit`.
  - 现象: 加了 `-quit` 会导致 Unity 在编译/导入后直接退出,测试不执行,也不会生成 `-testResults` XML.
  - 正确姿势: 让 TestRunner 自己在 run 完后 `EditorApplication.Exit(...)`.

---

## 2026-02-24: SceneView UI(overlay)上滑动仍闪烁的补强实现(ActiveCameraOnly/EditMode)

### 现象(用户反馈)
- 鼠标在 SceneView 的 UI(overlay/toolbar/tab)区域滑动时,仍会出现整体“显示/不显示”闪烁.

### 新推断(更偏 Editor 信号层)
- `UnityEditor.EditorWindow.mouseOverWindow` 在 SceneView 的部分 UI 区域里可能:
  - 返回 `null`,或
  - 返回“非 SceneView 的内部窗口”.
- 在 `mouseOverWindow==null` 且我们沿用缓存 hint 的情况下:
  - 如果缓存 hint 恰好是 GameView,ActiveCameraOnly 会把 ActiveCamera 解析为 Game/VR,
    从而导致 SceneView 那一帧没有提交 draw,表现为闪烁.

### 修复落点(本包)
- `Runtime/GsplatSorter.cs`:
  - 通过反射可选读取 `UnityEditor.SceneView.mouseOverWindow`(若存在),作为 `EditorWindow.mouseOverWindow` 的补强信号.
  - 并把该补强判定放在 “over==null -> 沿用缓存 hint” 之前,优先确保 SceneView 侧稳定.
- `Runtime/GsplatRendererImpl.cs`:
  - EditMode + ActiveCamera=SceneView 时,渲染目标相机改为遍历 `UnityEditor.SceneView.sceneViews` 的 cameras.
  - 避免 `Camera.GetAllCameras` 在 Editor 下拿不到隐藏 SceneView camera 时,退化为“只画某一个实例”导致闪烁.
...
## 2026-02-24: Systematic Debugging 结论(阶段性) - 改变优化策略,停止 EditMode 的自动切换

### 触发条件
- 多轮补丁后用户仍反馈: SceneView 的 UI(overlay/UIElements)区域滑动时仍闪烁.
- 这已经满足 systematic-debugging 的“3+ 次修复失败 -> 需要质疑架构”信号.

### 根因假设(架构层面,而非单点 bug)
- `ActiveCameraOnly(EditMode)` 试图根据 Editor UI 信号去判断 “你正在看 SceneView 还是 GameView”.
- 但这些信号在 overlay/UIElements 等路径里不可控,因此:
  - 只要我们继续做“自动切换”,就不可避免会遇到漏判或抖动.
  - 漏判一旦发生,渲染门禁会让 SceneView 当帧没有 draw,体感就是整体“显示/不显示”闪烁.

### 新策略(确定性契约)
- Editor 非 Play 模式下:
  - `ActiveCameraOnly` 默认 **始终选择 SceneView**(若存在)作为 ActiveCamera.
  - 不再基于 `focusedWindow` / `mouseOverWindow` 等信号自动切换到 GameView.
  - 想在 EditMode 看 GameView:
    - 用 `GsplatActiveCameraOverride` 显式指定 Game/VR 相机,或切到 `AllCameras`.
...
## 2026-02-24: 为“仍然闪烁但没有 log”增加证据采集链路

### 背景
- 用户继续反馈: 仍然闪烁,但 Console 没有可用线索.
- 这意味着我们缺少“相机渲染回调 vs draw 提交”的时序证据.

### 落地
- 增加 settings 开关:
  - `GsplatSettings.EnableEditorDiagnostics`(Project Settings/Gsplat 可勾选).
- 增加诊断工具:
  - `Runtime/GsplatEditorDiagnostics.cs`:
    - 环形缓冲记录近 512 条关键事件.
    - 自动检测并 dump:
      - “SceneView 相机触发渲染回调,但当帧没有提交 draw”.
- 接入点:
  - `Runtime/GsplatSorter.cs`: `OnPreCull` / `beginCameraRendering` 记录相机回调与 sort skip/dispatch.
  - `Runtime/GsplatRendererImpl.cs`: 记录 render skip 原因与 draw 提交目标相机.

### 目的
- 让我们能区分三类根因:
  1) draw 根本没提交(时序/门禁).
  2) draw 提交到错误相机实例(实例抖动/枚举不全).
  3) draw 提交了但被 Metal 跳过(缺失 buffer 或其它图形 API 约束).

---

## 2026-02-24 10:01:00 +0800: 闪烁复现后的日志证据(来自 Editor-prev.log)

### 证据1: Metal 跳绘制 warning 仍然存在
- 文件: `~/Library/Logs/Unity/Editor-prev.log`
- 关键行(约在 01:30:45 附近):
...
- 解释:
  - Unity 在 Metal 下为了避免崩溃,会直接跳过 draw call.
  - 这类跳绘制通常只会打印一次,后续可能静默,因此用户会感觉"没有 log"但仍然闪烁.

### 证据2: `[GsplatDiag]` dump 确实产出(菜单触发)
- 文件: `~/Library/Logs/Unity/Editor-prev.log`
- 关键行:
  - `[GsplatDiag] DUMP: reason=menu frame=1184`
  - `renderedSceneViews=[-31968]`
  - `drawnSceneViews=[-31968]`
- 解释:
  - dump 时刻,我们确实对 SceneView camera 提交了 draw.
  - 这意味着"闪烁"不一定是 "没有提交 draw".
  - 更可能是 "提交了但被 Metal 因 buffer 绑定问题跳过".

### 从 ring buffer 看到的现象(辅助)
- `BeginCameraRendering` 同一帧可能出现多次,并交替出现:
  - `[SORT] ... activeGsplats=1`
  - `[SORT_SKIP] ... reason=GatherGsplatsForCamera returned false`
- 暂时解读:
  - 这更像是 SRP 回调的多阶段/多次触发导致的“二次进入”现象.
  - 目前没有直接证据表明它会导致整体消失.

### 下一步结论
- 需要把诊断触发条件从 "没提交 draw" 扩展到 "捕获 Metal 跳绘制 warning".
- 修复上,需要让 StructuredBuffer 的绑定在 Metal 下稳定:
  - 仅靠 `MaterialPropertyBlock.SetBuffer` 可能不稳.
  - 候选: per-renderer material instance + `Material.SetBuffer` 作为稳定绑定源.
