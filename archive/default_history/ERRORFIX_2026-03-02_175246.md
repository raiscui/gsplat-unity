...
## 2026-03-03 01:08:00 +0800 - RadarScan 未响应 Show/Hide 动画

### 问题
- `RadarScan` 模式下点击 `Show/Hide`,雷达点云缺少与 `ParticleDots` 类似的显隐过程.

### 原因
- LiDAR 渲染链路只接了雷达开关可见性 `_LidarVisibility`.
- 未接入 show/hide runtime 状态(`m_visibilityState/progress/sourceMask`)与中心扩散掩码语义.

### 修复
- Renderer 侧新增 LiDAR overlay 参数构建函数,按当前 show/hide 状态输出:
  - gate/mode/progress/sourceMask
  - center/maxRadius/ringWidth/trailWidth
- LiDAR shader 新增 show/hide 掩码逻辑:
  - show: `max(primary, source)`
  - hide: `primary * source`
- Hidden 终态下直接跳过 LiDAR draw.
...
## 2026-03-03 01:38:00 +0800: RadarScan show/hide 无 noise 粒子感

### 问题
- `RadarScan` 下 show/hide 动画虽然存在,但边界过于平滑,看起来像纯几何遮罩,没有 `ParticleDots` 的 noise 粒子感.

### 原因
- `GsplatLidarScan.RenderPointCloud(...)` 已接收 noise 参数,但 `GsplatLidar.shader` 调用路径未完整使用:
  - primary/source mask 调用仍是旧签名.
  - ring glow 未统一走 noise jitter 计算.
- 结果是“参数传到了 MPB,但核心可见性计算没吃到参数”.

### 修复
- `Runtime/Shaders/GsplatLidar.shader`
  - `EvalLidarShowHideVisibleMask` 统一 show/hide 的 noise 影响.
  - `EvalLidarShowHideRingMask` 新增 mode,并改成外侧 ring + noise jitter.
  - vert 中对 primary/source/ring 全链路接入 `noise01/noiseSigned/noiseStrength`.
- `Tests/Editor/GsplatVisibilityAnimationTests.cs`
  - 新增 `LidarRenderPointCloud_Signature_ContainsShowHideNoiseParams` 锁定参数契约.

### 验证
- Unity EditMode `Gsplat.Tests`: total=43, passed=41, failed=0, skipped=2.
- 结果文件:
  - `/Users/cuiluming/local_doc/l_dev/my/unity/_tmp_gsplat_pkgtests/Logs/TestResults_lidar_showhide_noise_2026-03-03_0131_noquit.xml`

### 预防复发
- 后续若扩展 LiDAR overlay uniforms,必须同步检查:
  1. C# 参数传递(`RenderPointCloud` 参数)
  2. MPB 赋值
  3. shader 主路径(primary/source/ring)是否全部消费该参数
- 任何“参数已定义但视觉无变化”的问题,优先排查 shader 调用签名是否仍为旧版.

## 2026-03-03 02:07:00 +0800: RadarScan noise 已接线但观感仍不可见

### 问题
- 代码链路显示 noise 已传到 LiDAR shader,但现场观感几乎看不到噪声.

### 原因
- 不是参数丢失,而是公式过保守:
  - 采样频率偏低.
  - jitter 仅随 trailWidth 线性缩放,trail 小时几乎无扰动.

### 修复
- 提高噪声域频率.
- Curl 模式加入轻量 domain warp 和双层噪声混合.
- jitter 增加最小幅度下限并提高边界区域权重底线.
- ring glow 加入 noise 调制,保证前沿有可见颗粒明暗变化.

### 验证
- Unity EditMode `Gsplat.Tests`: total=43, passed=41, failed=0, skipped=2.
- 结果: `/Users/cuiluming/local_doc/l_dev/my/unity/_tmp_gsplat_pkgtests/Logs/TestResults_lidar_showhide_noise_tune2_2026-03-03_0204_noquit.xml`

## 2026-03-03 02:15:00 +0800: RadarScan noise 可见性不足(默认参数下)

### 现象
- 链路已经接通,但默认参数下仍觉得噪声不明显.

### 处理
- LiDAR 路径内部将 `noiseStrength` 映射提高到 `x1.6`.
- 保持外部参数接口不变,只增强 LiDAR 感知强度.
- 清理一个变量命名冲突隐患(提高 shader 可维护性).
...
## 2026-03-03 02:23:00 +0800: 参考 ParticleDots 后发现 LiDAR 缺少位置噪声

### 现象
- 即便接了边界 noise mask,用户仍反馈"看不到 noise".

### 根因
- `ParticleDots` 的噪声观感来自"遮罩 + 位置扰动".
- LiDAR 之前主要是遮罩噪声,缺少位移抖动,因此肉眼感知弱.

### 修复
- 在 `GsplatLidar.shader` 的 show/hide 路径新增粒子位移扰动(screen-space),并按前沿权重控制.
