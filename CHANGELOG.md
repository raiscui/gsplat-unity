...
- Added optional LiDAR "base intensity" controls to prevent points from turning black after a sweep: `LidarKeepUnscannedPoints` and `LidarUnscannedIntensity` (keeps a background brightness floor for bins far from the scan head).
- Added optional LiDAR distance attenuation controls for intensity falloff (near stronger, far weaker): `LidarIntensityDistanceDecayMode`, `LidarIntensityDistanceDecay`, and `LidarUnscannedIntensityDistanceDecay` (Reciprocal: `atten(dist)=1/(1+dist*decay)`, Exponential: `atten(dist)=exp(-dist*decay)`, and `decay=0` disables attenuation).
...
- Added controllable frustum external GPU capture resolution for RadarScan meshes: `LidarExternalCaptureResolutionMode` (`Auto` / `Scale` / `Explicit`), `LidarExternalCaptureResolutionScale`, and `LidarExternalCaptureResolution`, so external depth/color capture precision can be tuned independently from the LiDAR beam grid. `Scale > 1` is the recommended low-risk mitigation for external depth stair-stepping, while still preserving point-based nearest-surface resolve semantics.
- Added frustum external hybrid resolve controls for RadarScan meshes: `LidarExternalEdgeAwareResolveMode` (`Off` / `Kernel2x2` / `Kernel3x3`) and `LidarExternalSubpixelResolveMode` (`Off` / `Quad4`). These quality paths can be enabled independently or combined, keep final color aligned with the resolved depth winner, and still avoid blur / naive bilinear depth mixing.
...
- Fixed HDRP LiDAR `AlphaToCoverage` / `AnalyticCoveragePlusAlphaToCoverage` being incorrectly forced back to `AnalyticCoverage` just because `Camera.allowMSAA` is false; A2C availability now follows resolved HD Frame Settings / MSAA mode (and still respects the actual target texture sample count).
