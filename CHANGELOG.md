...
- Added controllable frustum external GPU capture resolution for RadarScan meshes: `LidarExternalCaptureResolutionMode` (`Auto` / `Scale` / `Explicit`), `LidarExternalCaptureResolutionScale`, and `LidarExternalCaptureResolution`, so external depth/color capture precision can be tuned independently from the LiDAR beam grid.
